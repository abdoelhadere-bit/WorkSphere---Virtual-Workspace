...
roleFilter.addEventListener('change', () => {
    const role = document.querySelector('select[name="roleFilter"]').value
    const filteredEmployees = filterByRole(employes, role)
    if(role === 'all'){
...
function displayEmployees(employees){
    ...
    if(employees.length === 0){
        employesContainer.innerHTML = '<span class="text-gray-500">Aucun employé disponible </span>'
        return
    }
    employees.forEach(emp => {
        ...
        element.innerHTML = `
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-500 truncate">${emp.localisation}</p>
            </div>
        `;
        ...
        employesContainer.appendChild(element);
